module tb;
  ...
  initial begin
    sw = 16'h1231; #1
    if (!(led === 16'h1230)) begin $display("FAIL: inverter output (led=%h seg=%b an=%b)", led, seg, an); $finish; end
    sw = 16'h0000; #1
    if (!(led === 16'h0001)) begin $display("FAIL: inverter output (led=%h seg=%b an=%b)", led, seg, an); $finish; end
    $display("PASS"); $finish;
  end
endmodule
