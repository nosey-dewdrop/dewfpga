module tb;
  ...
  initial begin
    sw = 16'h3A5C; #1
    if (!(led === 16'h00CA)) begin $display("FAIL: vector 1 (led=%h seg=%b an=%b)", led, seg, an); $finish; end
    sw = 16'hD5C9; #1
    if (!(led === 16'h0335)) begin $display("FAIL: vector 2 (led=%h seg=%b an=%b)", led, seg, an); $finish; end
    $display("PASS"); $finish;
  end
endmodule
