...
check "module name != file name -> clear error" "mkdir -p '$T/mn' && sed 's/module blink/module top/' '$ROOT/templates/blink.sv' > '$T/mn/lab4.sv' && cp '$ROOT/templates/blink.xdc' '$T/mn/lab4.xdc' && cd '$T/mn' && ! '$CLI' bit >out 2>&1 && grep -q \"no 'module lab4'\" out"
