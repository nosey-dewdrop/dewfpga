...
- "Oldu" demeden önce ölç: `test/run.sh` (51), `sim/test/e2e.mjs` (31, gerçek Chromium), CI clean install. Kartlı flash sadece kart takılıyken; test suite kartı görürse flash testini atlar.
...
- `docs/manual-setup.md` → `docs/build.sh` (html) + `docs/pdf.sh` (Chrome, 12 sayfa). 9 bölüm, her komutun gerçek çıktısı.
...
- **Gerçek repo turu (21 Eyl):** GitHub'dan 94 CS223 reposu klonlandı, 81 gerçek lab klasörü CLI'dan geçirildi (harness `scratchpad/corpus_run.py`, sonuçlar `corpus_results*.json`). Başta 2 lab derleniyordu, şimdi 8; kalan 73'ün hiçbiri CLI'ın suçu değil: 33'ü sadece testbench klasörü, 23'ü kendi pin dosyasını istiyor, 17'si öğrencinin kodundaki Vivado'nun kabul edip Yosys'in etmediği şeyler (isimsiz instance, always_comb latch, async reset sabit değil, iki saat kenarı) ve hepsi satır+düzeltmeyle söyleniyor, 5'i Vivado'nun ürettiği netlist dosyası (adıyla tanınıyor). Top artık hiyerarşiden bulunuyor (Vivado gibi: kimsenin instantiate etmediği modül), testbench içerikten (portsuz ya da `$finish`), dosya adı serbest; yedi-segment port satırı yerinde düzeltilip not basılıyor. Test paketi 51.
